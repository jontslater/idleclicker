...
# Station data
# Add 'manager_level' to each station
stations = [
...
while running:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            for i, station in enumerate(stations):
                ...
                if mgr_rect.collidepoint(event.pos) and score >= station["manager_cost"]:
                    ...
                    station["manager_cost"] = int(station["manager_cost"] * 1.5)  # Increase cost for next time
    ...
    mgr_label = font.render(f"Manager Lv{station['manager_level']} (${station['manager_cost']})", True, black)
